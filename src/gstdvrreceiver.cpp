...
namespace pipeline {
    static std::string create_qtdemux(){
        return "qtdemux ! ";
        assert(false);
        return "";
    }
...
GstDvrReceiver::GstDvrReceiver(char* dvr_file, const VideoCodec& codec)
{
    m_dvr_file=dvr_file;
    m_video_codec=codec;
...
static void loop_pull_appsink_samples(bool& keep_looping,GstElement *app_sink_element,
                                      const GstDvrReceiver::NEW_FRAME_CALLBACK out_cb){
...
std::string GstDvrReceiver::construct_gstreamer_pipeline()
{
    std::stringstream ss;
    ss<<"filesrc location="<<m_dvr_file<<" ! ";
    ss<<pipeline::create_qtdemux();
...
void GstDvrReceiver::loop_pull_samples()
{
...
void GstDvrReceiver::on_new_sample(std::shared_ptr<std::vector<uint8_t> > sample)
{
    if(m_cb){
        //debug_sample(sample);
        m_cb(sample);
...
void GstDvrReceiver::start_receiving(NEW_FRAME_CALLBACK cb)
{
    std::cout<<"GstDvrReceiver::start_receiving begin"<<std::endl;
    ...
    m_pull_samples_thread=std::make_unique<std::thread>(&GstDvrReceiver::loop_pull_samples, this);

    qDebug()<<"GstDvrReceiver::start_receiving end";
}

void GstDvrReceiver::stop_receiving()
{
