...
if [ $SKIP_SETUP -lt 1 ]; then
    ...
    case $BUILD_TYPE in
        deb)
            apt-get install -y cmake build-essential git pkg-config devscripts equivs librga-dev libgbm-dev
            ;;
        bin|debug)
            apt-get install -y cmake build-essential git pkg-config librockchip-mpp-dev libdrm-dev libpng-dev libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev libspdlog-dev nlohmann-json3-dev libmsgpack-dev libgpiod-dev libyaml-cpp-dev librga-dev libgbm-dev
            ;;
        test)
            apt-get install -y cmake build-essential git pkg-config librockchip-mpp-dev libdrm-dev libpng-dev libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev libspdlog-dev nlohmann-json3-dev libmsgpack-dev libgpiod-dev libyaml-cpp-dev catch2 librga-dev libgbm-dev
    esac
fi
